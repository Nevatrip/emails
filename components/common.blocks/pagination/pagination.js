        ...
        js: {
          inited() {
            ['prev', 'next'].forEach( direction => {
              const elem = this._elem( { elem: 'link', modName: 'to', modVal: direction } );
      ...
      move( direction ) {
        ['indicator', 'page'].forEach( item => {
          ...
          if( elem && elem.size() ) {
            switch( direction ) {
            case 'prev':
              elem.get( 0 ).domElem.insertAfter( elem.get( elem.size() - 1 ).domElem );
              break;
            case 'next':
              elem.get( elem.size() - 1 ).domElem.insertBefore( elem.get( 0 ).domElem );
              break;
            default:
              break;
            }
